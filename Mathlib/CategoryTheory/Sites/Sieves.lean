...
lemma ofArrows_category' {S : C} (R : Presieve S) :
  ...
  rw [Sieve.ofArrows, Presieve.ofArrows_category]
...
theorem functorPushforward_id (R : Sieve X) : R.functorPushforward (𝟭 _) = R := by
  rw [← generate_sieve R, ← generate_map_eq_functorPushforward]
  simp
...
theorem functor_galoisConnection (X : C) :
  ...
  intro R S
  change Presieve.functorPushforward F R.arrows ≤ S.arrows ↔
    R.arrows ≤ Presieve.functorPullback F S.arrows
  rw [← arrows_generate_map_eq_functorPushforward]
  exact (generate_le_iff _ _).trans Presieve.map_le_iff_le_functorPullback
